...
class GraphRetriever:
    ...
    def retrieve_blast_radius(self, service_name: str) -> dict:
        ...
        logger.debug("Blast radius for {}: {} downstream services", service_name, len(downstream))
        return result

    def retrieve_related_incidents(self, query: str, top_services: list[str]) -> list[dict]:
        ...
        logger.debug("Related incidents for {}: {} found", top_services, len(results))
        return results

    def retrieve_patterns(self, root_cause_category: str | None = None) -> dict:
            ...
            if attrs.get("type") == "Incident":
                ...
                for nb in g.successors(node):
                    ...
                    if nb_attrs.get("type") == "RootCause":
                        rc_counts[nb_attrs.get("category", "unknown")] += 1
            elif attrs.get("type") == "Service":
                incidents = self.graph.get_incidents_for_service(attrs.get("name", node))
                if incidents:
                    service_incident_counts[attrs.get("name", node)] = len(incidents)

        result = {
            "root_cause_distribution": dict(rc_counts.most_common()),
            "severity_distribution": dict(severity_counts.most_common()),
            "most_affected_services": dict(service_incident_counts.most_common(10)),
        }

        if root_cause_category:
            result["filtered_category"] = root_cause_category
            result["filtered_count"] = rc_counts.get(root_cause_category, 0)

        logger.debug("Pattern retrieval: {} root causes, {} severities",
                     len(rc_counts), len(severity_counts))
        return result

    def extract_service_names(self, query: str, known_services: list[str]) -> list[str]:
        """Find known service names mentioned in the query text."""
        q = query.lower()
        found = []
